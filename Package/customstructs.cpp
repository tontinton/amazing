...
bool C_BaseCombatWeapon::CanFire()
{
    ...
    float flServerTime = g_player->m_nTickBase() * Interfaces::g_globalVars->interval_per_tick;
...
{
    player_info_t info;
    Interfaces::g_engineClient->GetPlayerInfo(EntIndex(), &info);
    return info;
}
...
Vector C_BasePlayer::GetHitboxPos(int hitbox_id)
{
    ...
    if (SetupBones(boneMatrix, MAXSTUDIOBONES, BONE_USED_BY_HITBOX, 0.0f)) {
        auto studio_model = Interfaces::g_mdlInfo->GetStudiomodel(GetModel());
        if (studio_model) {
...
bool C_BasePlayer::GetHitboxPos(int hitbox, Vector& output)
{
    ...
    studiohdr_t* studioHdr = Interfaces::g_mdlInfo->GetStudiomodel(model);
    if (!studioHdr)
        return false;
